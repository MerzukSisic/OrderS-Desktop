...
class _StoresListScreenState extends State<StoresListScreen> {
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const Text('Delete Store', style: TextStyle(color: AppColors.textPrimary)),
          ],
        ),
        content: Text(
            ...
            onPressed: () async {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  content: Text(success ? 'Store deleted' : 'Failed to delete store'),
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16),
                          Text('No stores found', style: TextStyle(color: AppColors.textSecondary, fontSize: 18, fontWeight: FontWeight.w500)),
                        ],
                      ),
                    )
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) => _buildStoreCard(filtered[index]),
                    ),
            ),
          ],
        );
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                const Text('Stores Management', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                const SizedBox(height: 8),
                Text('$total total stores', style: TextStyle(color: AppColors.textSecondary.withValues(alpha: 0.7), fontSize: 14)),
              ],
            ),
          ),
          Container(
  ...
  Widget _buildStoreCard(Store store) {
    final statusColor = store.isActive ? AppColors.success : AppColors.textSecondary;
    return Container(
      ...
      child: ListTile(
        ...
        title: Row(
          children: [
            Text(store.name, style: const TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
            ...
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(
                color: (store.isExternal ? AppColors.warning : AppColors.primary).withValues(alpha: 0.15),
                borderRadius: BorderRadius.circular(6),
              ),
              child: Text(
          ...
          children: [
            if (store.location != null) ...[
              ...
              Row(children: [
                ...
                Text(store.location!, style: TextStyle(color: AppColors.textSecondary.withValues(alpha: 0.7))),
              ]),
            ],
            if (store.description != null) ...[
              const SizedBox(height: 2),
              Text(store.description!, style: TextStyle(color: AppColors.textSecondary.withValues(alpha: 0.7), fontSize: 12)),
            ],
          ],
        ),
        trailing: Row(
          ...
          children: [
            IconButton(
              ...
              onPressed: () => Navigator.pushNamed(context, AppRouter.adminStoreProducts, arguments: store),
              tooltip: 'Products',
            ...
            IconButton(
              ...
              onPressed: () async {
                await Navigator.pushNamed(context, AppRouter.adminStoreEdit, arguments: store);
                if (mounted) context.read<StoresProvider>().fetchStores();
              },
              tooltip: 'Edit',
